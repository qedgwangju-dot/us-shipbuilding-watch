    ...
    for idx, paragraph in enumerate(enriched.get("body_ko") or []):
        for sentence in base.split_sentences(paragraph):
            ...
            if re.search(r"\d", sentence):
                score += 3
            if any(x in sentence for x in ("발표", "결정", "합의", "면제", "재할당", "비축", "생산", "권리", "지분", "가격", "원유", "저장")):
                score += 2
            if idx == 0:
                score += 1
...
def concise_news_alert(topic, enriched, key, grouped_items):
    ...
    if snapshot:
        lines.extend(["", "<b>핵심</b>"])
        lines.extend(f"• {html.escape(x, quote=False)}" for x in snapshot[:3])

    impacts = impact_lines_v2(topic, key)
    if impacts:
        lines.extend(["", "<b>판정</b>"])
        lines.extend(f"• {html.escape(x, quote=False)}" for x in impacts[:2])
